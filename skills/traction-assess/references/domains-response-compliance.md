...
### 11.1 AI Data Governance
**Question**: Can customers control how their data is used by AI?

- **L1** — Usage documented: Clear documentation of whether and how customer data is used for training, inference, and improvement. Data retention periods and processing locations stated. For API consumers, this means a published policy on whether customer data is sent to third-party model providers and whether providers may use it for training.
  ...
  - Threats: `data_leakage`, `unauthorized_training`, `privacy_violation`
- **L2** — Opt-out with separation: Customers can opt out of data usage for training. Customer data separated from training pipelines. Cross-customer data exposure prevented. For API consumers, opt-out means contractual and technical controls ensuring the provider cannot use data for training.
  - Why: AIUC-1 A005 mandatory. GDPR Art. 21 right to object. Mid-market and regulated industry expectation.
- **L3** — Explicit opt-in, isolated: Customer data only used with explicit opt-in consent with timestamp. Isolated training pipelines per customer. Data lineage tracked from source to model. For API consumers, isolation means dedicated inference endpoints, VPC-scoped access.
...
- **L1** — Shared, no config: Single shared model, no customer configuration.
  ...
  - Threats: `vendor_lock_in`, `model_drift`
- **L2** — Model selection options: Customers can choose between different model options. For API consumers, this means the ability to choose between model providers or versions and pin specific model versions.
  - Why: Flexibility for different use cases.
- **L3** — Fine-tuning available: Customers can fine-tune models on their data. For API consumers, this includes RAG configuration, custom system prompts, and provider-supported fine-tuning.
  - Why: Custom performance. Domain-specific optimization.
- **L4** — Customer-managed deployment: Customers can deploy models in their own infrastructure. For API consumers, this means provisioned throughput, dedicated capacity, and customer-VPC endpoints.
...
- **L1** — Basic interaction logs: AI interactions logged with prompt (or hash), response summary, timestamp, model version, and errors. Viewable in application interface.
  ...
  - Threats: `undetected_misuse`, `audit_gap`, `incident_blindness`
...
- **L1** — Basic safety filters: PII detection in outputs. Harmful content blocking (toxicity, violence, self-harm). Vendor-provided safety filters for common risk categories.
  ...
  - Threats: `pii_leakage`, `harmful_content`, `hallucination`
...
- **L1** — Fully automated: AI operates without human review capability.
  ...
  - Threats: `automated_harm`, `bias_amplification`, `accountability_gap`
...
- **L1** — AI disclosure: Clear notice to users they are interacting with AI, visible before first interaction. AI-generated content clearly labeled.
  ...
  - Threats: `deceptive_ai`, `regulatory_violation`
...
- **L1** — Interaction history visible: Users can view their AI interaction history. Consent captured before first AI interaction with timestamp. Clear terms of service covering AI usage.
  ...
  - Threats: `privacy_violation`, `data_rights_violation`, `regulatory_noncompliance`
...
- **L1** — Providers documented: AI providers, specific models, and versions documented with rationale for selection. Known limitations per provider documentation acknowledged and communicated. For API consumers, this must include which providers process customer data, which models are used, and in which regions.
  ...
  - Threats: `supply_chain_compromise`, `vendor_lock_in`, `shadow_ai`
- **L2** — Vendor due diligence: Provider data handling and retention confirmed in writing. DPAs signed with all AI processors. Data processing locations documented. No training on customer data confirmed. For API consumers, this includes understanding provider model deprecation policies, sunset timelines.
...
- **L3** — Change management: Model updates, prompt changes, and data source changes require review and approval before deployment. Version tracking with rollback capability. Impact assessment for model changes. For API consumers, this includes model version pinning, testing against new provider model versions.
...
- **L1** — Unrestricted agent actions: AI agents can invoke any available tool or action without explicit authorization boundaries. No distinction between read and write operations. Agent runs with the user's full permissions.
  ...
  - Threats: `privilege_escalation`, `prompt_injection`, `api_abuse`, `data_exfiltration`
- **L2** — Allowlisted tools with risk tiers: Agents have an explicit tool allowlist. Actions categorized by risk: low-risk (reads) auto-approved, medium-risk (writes) logged, high-risk (delete, push, external API) require human approval. Agent permissions scoped to current task. Intent-based access control (IBAC) can enforce this by parsing user intent into fine-grained authorization tuples.
...
- **L3** — Delegation trust chains: Multi-agent systems enforce permission inheritance -- sub-agents inherit at most parent permissions, never exceed them. Delegation chains are auditable. Separation of duties: proposing agent cannot be approving agent. Compromised agent cannot poison swarm context. Authorization tuples should have temporal isolation (TTL-based expiry).
...
- **L4** — Customer-controlled agent policies: Customers define agent tool policies per role or use case. Real-time agent action monitoring with anomaly detection. Policy-as-code for agent authorization with fine-grained authorization engines (e.g., OpenFGA). Independent audit trail of all agent decisions separate from provider logs.
...
- **L1** — No memory controls: AI context/memory persists without explicit controls. No tenant isolation in conversation context. System prompts and user content not separated. Data removed from prompts may persist in provider logs or fine-tuning datasets.
  ...
  - Threats: `cross_tenant_leakage`, `memory_poisoning`, `information_remnance`, `context_injection`
...
### Domain 11 Reference Standards
- **AIUC-1**: Controls A001-A006 (Data Governance), B001-B006 (Input Security), C001-C006 (Output Safety), D001-D003 (Human Oversight), E001-E017 (Transparency & Operations)
- **Open Security Architecture**: Prompt Security, Memory & Persistence Security, Tool & Action Authorization, Agent Orchestration
- **MITRE ATLAS**: Adversarial threat landscape for AI systems
- **NIST AI RMF**: Risk management sub-controls for AI system governance
- **OWASP LLM Top 10**: Primary threat reference for input/output security controls
